...
for (let i = setup + 1; i < lines.length; i++) {
  ...
  //   console.log(parseInt(line[1]), parseInt(line[3]), parseInt(line[5])); // Logging
  for (
    let j = 0;
    j < parseInt(line[1]);
    j++ // Move j (times) from line[3] to line[5]
  )
    stacks[parseInt(line[5])].push(stacks[parseInt(line[3])].pop());
}
